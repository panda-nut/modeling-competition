...
sys.path.insert(0, str(ROOT / "output" / "question45"))
import q3_multiobjective_optimization as q3
import q45_preference_and_robustness as q45

OUT = Path(__file__).resolve().parent / "revision_data"
...
def main():
    ...
    anova=anova_contributions(pin); anova.to_csv(OUT/"anova_contributions.csv",index=False,encoding="utf-8-sig")
    cv,enc_models=fit_encoding_models(pin); cv.to_csv(OUT/"encoding_cv.csv",index=False,encoding="utf-8-sig")
    opt=[]
    ...
    pd.DataFrame(opt,columns=["模型","a","b","N","R","P","U","评分"]).to_csv(OUT/"model_reoptimization.csv",index=False,encoding="utf-8-sig")
    structured_holdout(pin).to_csv(OUT/"structured_holdout.csv",index=False,encoding="utf-8-sig")
    ...
    pref_sensitivity(ref_f,ref_x).to_csv(OUT/"preference_sensitivity.csv",index=False,encoding="utf-8-sig")
