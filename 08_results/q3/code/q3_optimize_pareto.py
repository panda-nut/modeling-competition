...
程序从附件2重新拟合问题2确定的三次 Ridge 模型，并把无针肋数据
作为独立 PCHIP 支路处理。
"""
...
def make_figures(x, f, candidates, ideal, nadir):
    ...
    ax.set_xticks(range(3),["R","P","U"]); ax.set_ylabel("统一归一化性能"); ax.grid(alpha=.25); ax.legend(ncol=2,fontsize=8); fig.tight_layout(); fig.savefig(FIG/"fig_q3_02_candidate_comparison.pdf",bbox_inches="tight"); plt.close(fig)
    # 最终点所属 N 切片的折中评分图。
    ...
    ax.set(xlabel="a",ylabel="b",title=f"N={int(n)} 的综合评分切片"); ax.legend(); fig.tight_layout(); fig.savefig(FIG/"fig_q3_03_design_slice.pdf",bbox_inches="tight"); plt.close(fig)
...
\\begin{{figure}}[H]\\centering\\includegraphics[width=.94\\linewidth]{{../figures/fig_q3_01_pareto_front.pdf}}\\caption{{全局Pareto前沿及两两投影}}\\end{{figure}}
\\begin{{figure}}[H]\\centering\\includegraphics[width=.86\\linewidth]{{../figures/fig_q3_02_candidate_comparison.pdf}}\\caption{{候选方案的统一归一化比较}}\\end{{figure}}
\\begin{{figure}}[H]\\centering\\includegraphics[width=.68\\linewidth]{{../figures/fig_q3_03_design_slice.pdf}}\\caption{{综合最优点所在排数层的局部评分}}\\end{{figure}}
\\section{{结论}} 三个目标的冲突使单一极端点不可作为综合设计。等权增强切比雪夫准则选择的方案控制了最大标准化退让；它可作为名义工况基准，权重变化和扰动鲁棒性应在后续问题中另行分析。\\end{{document}}'''
    DRAFT.mkdir(parents=True, exist_ok=True)
    (DRAFT/"q3_demo.tex").write_text(tex,encoding="utf-8")


def main():
    global RIDGE, PCHIP
    for d in [DATA,FIG,LOG,DRAFT]: d.mkdir(parents=True,exist_ok=True)
    ...
    pd.DataFrame(np.c_[x,f],columns=["a","b","N","R","P","U"]).to_csv(DATA/"q3_pareto_global.csv",index=False)
    payoff=pd.DataFrame(f[ids],index=["R锚点","P锚点","U锚点"],columns=RESP); payoff.to_csv(DATA/"q3_payoff_matrix.csv",encoding="utf-8-sig")
    ...
    # 演示性论文在 drafts 中独立维护；模型复现只写入结果数据产物，
    # 避免运行数值程序时覆盖未审核的论文本。
