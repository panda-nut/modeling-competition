...
sys.path.insert(0, str(ROOT / "output" / "question3"))
...
def make_figures(weights, pref, struct, physical):
    ...
    fig.tight_layout();fig.savefig(FIG/"q4_weight_mapping.pdf",bbox_inches="tight");plt.close(fig)
    ...
    ax.set_xticks(x,struct["方案"],rotation=15,ha="right");ax.set_ylabel("固定尺度评分");ax.set_title("结构加工误差下的风险比较");ax.legend();ax.grid(axis="y",alpha=.22);fig.tight_layout();fig.savefig(FIG/"q5_structure_risk.pdf",bbox_inches="tight");plt.close(fig)
    ...
    ax.set(ylabel="CVaR$_{0.95}(C_{op})$",title="工况机理情景下的鲁棒风险");ax.grid(alpha=.22);ax.legend();fig.tight_layout();fig.savefig(FIG/"q5_operation_envelope.pdf",bbox_inches="tight");plt.close(fig)
...
\begin{{figure}}[H]\centering\includegraphics[width=.92\linewidth]{{figures/q4_weight_mapping.pdf}}\caption{{权重变化到最优设计的映射}}\end{{figure}}
...
\begin{{figure}}[H]\centering\includegraphics[width=.88\linewidth]{{figures/q5_structure_risk.pdf}}\caption{{结构加工误差传播：名义代价与上尾风险}}\end{{figure}}
对于流量、热负荷扰动，使用$\Theta_R=s_qR^{{op}}$、$P^{{op}}$、$U^{{op}}$组成评分。S1--S3只表示黏性/惯性和流量敏感度的模型形式包络；U模型A/B反映温度非均匀性是否随热负荷幅值缩放，均不被解释为附件标定参数。\begin{{figure}}[H]\centering\includegraphics[width=.78\linewidth]{{figures/q5_operation_envelope.pdf}}\caption{{工况机理情景下的风险包络（B类辅助复核）}}\end{{figure}}
\section{{工程建议与边界}}
...
def write_odt(summary):
    ...
    path=OUT/"APMCM_B题_问题4与5_偏好及扰动鲁棒性.odt"
...
def main():
    ...
    ref=pd.read_csv(ROOT/"output"/"question3"/"data"/"pareto_reference_merged.csv")
    ...
    make_figures(weights,pref,struct,physical)
