...
DATA, FIG, LOG = OUT / "data", OUT / "figures", OUT / "logs"
...
def main():
    ...
    ref_df=pd.read_csv(DATA/"pareto_global.csv"); ref_x=ref_df[["a","b","N"]].to_numpy(); ref_f=ref_df[q3.RESP].to_numpy()
    ...
    metrics=pd.DataFrame(rows); metrics.to_csv(DATA/"algorithm_metrics.csv",index=False,encoding="utf-8-sig")
    pd.DataFrame(traces,columns=["seed","N","generation","min_R","min_P","min_U"]).to_csv(DATA/"nsga2_trace.csv",index=False)
    nx=np.vstack([v[0] for v in all_runs]); nf=np.vstack([v[1] for v in all_runs]); take=q3.nondominated(nf)
    pd.DataFrame(np.c_[nx[take],nf[take]],columns=["a","b","N","R","P","U"]).to_csv(DATA/"pareto_nsga2.csv",index=False)
    # 算法比较图：三级网格收敛 + 20次NSGA-II箱线图。
    fig,axs=plt.subplots(1,3,figsize=(10.8,3.25)); plt.rcParams.update({"font.sans-serif":["Microsoft YaHei","SimHei"],"axes.unicode_minus":False})
    g=metrics[metrics.method.str.contains("网格")]
    axs[0].plot([101,201,401],g.HV,"o-",color="#2563eb"); axs[0].set(xlabel="网格边长",ylabel="HV",title="网格收敛"); axs[0].grid(alpha=.25)
    ns=metrics[metrics.method=="分层 NSGA-II"]
    axs[1].boxplot(ns.HV,labels=["NSGA-II"]); axs[1].axhline(g.iloc[-1].HV,color="#dc2626",ls="--",label="401网格"); axs[1].set(ylabel="HV",title="20次重复的超体积"); axs[1].legend(fontsize=8)
    axs[2].boxplot(ns.IGD_plus,labels=["NSGA-II"]); axs[2].axhline(g.iloc[-1].IGD_plus,color="#dc2626",ls="--"); axs[2].set(ylabel="IGD+",title="20次重复的逼近误差")
    fig.tight_layout(); fig.savefig(FIG/"q3_algorithm_comparison.pdf",bbox_inches="tight"); plt.close(fig)
