...
sys.path.insert(0,str(ROOT/'output'/'question3')); import q3_multiobjective_optimization as q3

OUT,DATA=HERE,HERE/'data'
...
def main():
    ...
    ref=pd.read_csv(ROOT/'output'/'question3'/'data'/'pareto_reference_merged.csv'); rx=ref[['a','b','N']].to_numpy(float); rf=ref[['R','P','U']].to_numpy(float)
    ...
    fig,ax=plt.subplots(figsize=(8.0,3.7));im=ax.imshow(np.abs(sens.iloc[:,2:6].to_numpy(float)),cmap='YlOrRd',aspect='auto');ax.set_xticks(range(4),sens.columns[2:6]);ax.set_yticks(range(len(sens)),sens['扰动因素']+' ('+sens['类别']+')');fig.colorbar(im,ax=ax,label='|标准化回归系数|');
    for i in range(len(sens)):
        for j in range(4):ax.text(j,i,f'{abs(sens.iloc[i,j+2]):.2f}',ha='center',va='center',fontsize=8)
    ax.set_title('扰动因素的统计敏感性（结构鲁棒方案）');fig.tight_layout();fig.savefig(HERE/'figures'/'q5_sensitivity.pdf',bbox_inches='tight');plt.close(fig)
    # 扩充Tex。
...
H=50下$N=2,4,6,8,10$的权重区域占比分别为{', '.join(f'{v:.1%}' for v in pis['权重区域占比'])}；$a^*$范围为[{d.a.min():.4f},{d.a.max():.4f}]，$b^*$范围为[{d.b.min():.4f},{d.b.max():.4f}]。无针肋从未成为最优，说明其压降优势不足以抵消热阻和非均匀性损失。\begin{{figure}}[H]\centering\includegraphics[width=.88\linewidth]{{figures/q4_weight_mapping.pdf}}\caption{{权重到最优设计的映射；右图颜色表示$b^*$}}\end{{figure}}
...
结构鲁棒方案为$({sx[0]:.6f},{sx[1]:.6f},{int(sx[2])})$；相对\emph{{名义可行投影}}的名义代价为{(risk.iloc[3].C_nom/nomc-1):.2%}，CVaR降幅为{(nomr-strr)/nomr:.2%}；样本量加倍后CVaR变化为{risk.iloc[3]['收敛差']:.2e}。问题3名义点位于$b=4.5$边界，不能接受对称正向公差，表中仅将其作为越域诊断，不将其CVaR与鲁棒可行设计作正式比较。\begin{{figure}}[H]\centering\includegraphics[width=.86\linewidth]{{figures/q5_sensitivity.pdf}}\caption{{结构鲁棒方案的统计敏感性：加工误差与S2-A工况情景分别计算}}\end{{figure}}
...
S1=(.80,.75,.25,.25)$，S2=(.50,.50,.50,.50)$，S3=(.20,.25,.80,.80)$，元组顺序为$(\alpha_P,\theta_R,m_R,m_U)$。六个情景的最优设计在$N=4$与$N=10$间分裂，故缺少变工况数据时不能给出唯一的物理鲁棒设计；应按U的尺度解释分别选择表中方案。工况结论仅为B类机理包络，不与A类结构鲁棒结果混同。\end{{document}}'''
    ...
    tex=tex.replace('%', r'\%').replace('figures/', '../figures/')
    ...
    (OUT/'APMCM_B题_问题4与5_偏好及扰动鲁棒性.tex').write_text(tex,encoding='utf-8')
