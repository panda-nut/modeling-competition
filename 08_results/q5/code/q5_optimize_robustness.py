...
\begin{{figure}}[H]\centering\includegraphics[width=.92\linewidth]{{../../q4/figures/fig_q4_01_weight_mapping.pdf}}\caption{{权重变化到最优设计的映射}}\end{{figure}}
\section{{问题5：加工误差与工况波动}}
中等公差情景取$\varepsilon_a,\varepsilon_b\in[-0.025,0.025]$，从而在鲁棒可行域$a\in[0.105,0.295]$、$b\in[3.0375,4.4625]$内重新搜索。对$10^4$量级LHS实现样本，结构加工鲁棒目标为$\min_x\mathrm{{CVaR}}_{{0.95}}[C_{{str}}]$。得到结构加工误差分析结果
\[x_{{str}}^{{rob}}=({st.a:.6f},{st.b:.6f},{int(st.N)}),\quad \mathrm{{CVaR}}_{{0.95}}={st.CVaR95_C:.6f}.\]
\begin{{figure}}[H]\centering\includegraphics[width=.88\linewidth]{{../figures/fig_q5_01_structure_risk.pdf}}\caption{{结构加工误差传播：名义代价与上尾风险}}\end{{figure}}
对于流量、热负荷扰动，使用$\Theta_R=s_qR^{{op}}$、$P^{{op}}$、$U^{{op}}$组成评分。S1--S3只表示黏性/惯性和流量敏感度的模型形式包络；U模型A/B反映温度非均匀性是否随热负荷幅值缩放，均不被解释为附件标定参数。\begin{{figure}}[H]\centering\includegraphics[width=.78\linewidth]{{../figures/fig_q5_02_operation_envelope.pdf}}\caption{{工况机理情景下的风险包络}}\end{{figure}}
\section{{工程建议与边界}}
...
def main():
    ...
    # drafts 中的论文本与模型数据产出解耦，复现不自动改写演示稿。
