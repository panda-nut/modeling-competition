"""问题4--5最终证据边界修订：不以越域Ridge外推生成正式风险结论。"""
...
sys.path[:0]=[str(OUT),str(ROOT/'output'/'question3')]
import q45_preference_and_robustness as q
...
def main():
    data=OUT/'data'; figs=OUT/'figures'
    pin,base=q3.read_data(); ridge,pchip=q3.build_surrogate(pin,base)
    # 已由20k样本优化得到的域内设计；只在鲁棒可行域内报告正式风险。
    summary=json.loads((data/'q45_revision_summary.json').read_text(encoding='utf-8'))
    ...
    fig,ax=plt.subplots(figsize=(8,3.7)); lim=max(.05,float(np.abs(signed.to_numpy()).max()));im=ax.imshow(signed,cmap='RdBu_r',vmin=-lim,vmax=lim,aspect='auto')
    ax.set_xticks(range(4),signed.columns);ax.set_yticks(range(6),signed.index);fig.colorbar(im,ax=ax,label='带符号标准化回归系数 (SRC)')
    for i in range(6):
        for j in range(4):ax.text(j,i,f'{signed.iloc[i,j]:+.2f}',ha='center',va='center',fontsize=8)
    ax.set_title('扰动敏感性的方向与强度（结构鲁棒方案）');fig.tight_layout();fig.savefig(figs/'q5_signed_sensitivity.pdf',bbox_inches='tight');plt.close(fig)
...
无针肋在1326个$H=50$权重下从未成为最优；$N=2,4,6,8,10$的区域占比分别为3.8%、55.8%、22.7%、2.6%、15.0%。$N=4$覆盖权重区域最多，而$N=6$最小化最坏机会损失；前者衡量场景出现区域，后者衡量跨场景最坏损失，二者标准不同并不矛盾。\begin{{figure}}[H]\centering\includegraphics[width=.86\linewidth]{{../figures/q4_weight_mapping.pdf}}\caption{{权重到最优设计的映射；右图颜色表示$b^*$}}\end{{figure}}
...
带符号SRC由$\mathrm{{SRC}}_{{kj}}=\hat\beta_{{kj}}\sigma_{{X_j}}/\sigma_{{Y_k}}$计算；表中$R^2$为相应标准化线性回归的解释度。加工误差中$\varepsilon_b$主导综合评分；S2-A中$s_q$主要正向放大热目标与综合风险，$s_m$使压降正向增加而使热目标和$U$下降。加工分析的热目标为$R$，工况分析的热目标为$\Theta_R$。\begin{{figure}}[H]\centering\includegraphics[width=.86\linewidth]{{../figures/q5_signed_sensitivity.pdf}}\caption{{带符号SRC；第一列在加工分析中为$R$、工况分析中为$\Theta_R$}}\end{{figure}}
