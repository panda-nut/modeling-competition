...
def make_nsga_runs(ridge,pchip):
    ...
    ans.to_csv(DATA/"nsga2_runs.csv",index=False); return ans
...
def plot_slice(ridge,pchip,ideal,nadir,ref_df,final):
    ...
    fig.tight_layout();fig.savefig(FIG/"q3_final_design_slice.pdf",bbox_inches="tight");plt.close(fig)
...
def main():
    ...
    local=continuous_refinement(ridge,pchip,ideal,nadir,grid,nsnd);local.to_csv(DATA/"local_refinement.csv",index=False,encoding="utf-8-sig")
    union=pd.concat([grid,nsnd,local[["a","b","N","R","P","U"]]],ignore_index=True)
    take=q3.nondominated(union[q3.RESP].to_numpy());ref=union[take].reset_index(drop=True);ref.to_csv(DATA/"pareto_reference_merged.csv",index=False)
    ...
    metrics=pd.DataFrame(rows,columns=["method","run","points","HV","IGD_plus","Spacing","evaluations"]);metrics.to_csv(DATA/"algorithm_metrics_exact.csv",index=False,encoding="utf-8-sig");plot_algorithm_comparison(metrics)
    ...
    plot_pareto(ref,base_curve,anchors,final);plot_slice(ridge,pchip,ideal,nadir,ref,final)
