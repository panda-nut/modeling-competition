"""问题4--5综合分析：评分闭环、遗憾比较、灵敏度及工况设计判定。"""
...
sys.path.insert(0,str(HERE)); import q4_optimize_preferences as q
sys.path.insert(0,str(ROOT/'08_results'/'q3'/'code')); import q3_optimize_pareto as q3

OUT,DATA=ROOT/'08_results'/'q4'/'drafts',q.DATA
...
def main():
    ...
    ref=pd.read_csv(ROOT/'08_results'/'q3'/'data'/'q3_pareto_reference_merged.csv'); rx=ref[['a','b','N']].to_numpy(float); rf=ref[['R','P','U']].to_numpy(float)
...
\title{{APMCM B题问题4--5：偏好与扰动鲁棒性}}\author{{}}\date{{}}\begin{{document}}\maketitle
...
S1=(.80,.75,.25,.25)$，S2=(.50,.50,.50,.50)$，S3=(.20,.25,.80,.80)$，元组顺序为$(\alpha_P,\theta_R,m_R,m_U)$。六个情景的最优设计在$N=4$与$N=10$间分裂，故缺少变工况数据时不能给出唯一的物理鲁棒设计；应按U的尺度解释分别选择表中方案。工况结论仅为B类机理包络，不与A类结构鲁棒结果混同。\end{{document}}'''
    ...
    (DATA/'q45_robustness_summary.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2),encoding='utf-8')
