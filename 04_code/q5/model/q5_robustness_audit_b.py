...
OUTPUT = ROOT / "03_data" / "processed" / "q5"
...
def distribution_audit(designs: np.ndarray, ridge, pchip) -> pd.DataFrame:
    rows = []
...
def model_ambiguity_audit(designs: np.ndarray, ridge, pchip) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    scenario_rows.append(
        {
            "selection": "minimax_model_regret_b",
            "scenario": "all_six",
...
def sobol_audit(ridge, pchip, design: np.ndarray, sample_power: int = 15) -> tuple[pd.DataFrame, pd.DataFrame]:
    n = 2**sample_power
...
def main() -> None:
    ...
    distributions = distribution_audit(designs, ridge, pchip)
    domain = domain_audit()
    ambiguity, scenario_risks = model_ambiguity_audit(designs, ridge, pchip)
    robust_layer = pd.read_csv(STRUCTURE_OPTIMA)
    robust = robust_layer.iloc[int(robust_layer.CVaR95.argmin())][["a", "b", "N"]].to_numpy(float)
    sobol, dominant = sobol_audit(ridge, pchip, robust)
    OUTPUT.mkdir(parents=True, exist_ok=True)
    outputs = {
        OUTPUT / "q5_error_distribution_comparison_b.csv": distributions,
        OUTPUT / "q5_perturbation_domain_audit_b.csv": domain,
        OUTPUT / "q5_model_ambiguity_robust_b.csv": ambiguity,
        OUTPUT / "q5_cross_scenario_regret_b.csv": scenario_risks,
        OUTPUT / "q5_sobol_indices_b.csv": sobol,
        OUTPUT / "q5_dominant_metric_switch_b.csv": dominant,
    }
