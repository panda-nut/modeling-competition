...
def plot_main_effects(main_effects: pd.DataFrame) -> None:
    ...
    figure.suptitle("[B] 结构变量对三项响应的主效应（相对总体均值）")
    figure.tight_layout()
    save_both(figure, "fig_q1_01_main_effects_b")
...
def plot_tradeoffs(response_data: pd.DataFrame) -> None:
    ...
    figure.suptitle("[B] 有针肋样本与无针肋退化基线的性能权衡")
    figure.subplots_adjust(left=0.065, right=0.90, bottom=0.17, top=0.84, wspace=0.30)
    save_both(figure, "fig_q1_02_performance_tradeoff_b")
...
def plot_contributions(contributions: pd.DataFrame) -> None:
    ...
    figure.suptitle("[B] 无重复全因子设计的描述性效应贡献（不作显著性检验）")
    figure.tight_layout()
    save_both(figure, "fig_q1_03_effect_contributions_b")
...
def plot_interactions(interactions: pd.DataFrame) -> None:
    ...
    figure.suptitle("[B] 关键二阶交互的条件均值曲线")
    figure.tight_layout()
    save_both(figure, "fig_q1_04_interaction_effects_b")


def main() -> None:
    configure()
    FIGURE.mkdir(parents=True, exist_ok=True)
    main_effects = pd.read_csv(DATA / "q1_main_effects_b.csv")
    interactions = pd.read_csv(DATA / "q1_interaction_effects_b.csv")
    contributions = pd.read_csv(DATA / "q1_effect_contributions_b.csv")
    ...
    print("wrote Q1 [B] PDF and PNG figures")
